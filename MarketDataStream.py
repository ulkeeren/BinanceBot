...
class MarketDataFetcher:
    def __init__(self,api_key=0,api_secret=0 ):
        ...
        self.whitelist = open("whitelist.txt","r")
    ...
    def updateWhitelistData(self,pair_in,interval_in):
        ...
        time_diff,begin_time = self.calculateTimeDifferenceAsInterval(pair_in,interval_in)
        data = pd.read_csv(self.returnPairFile(pair_in, interval_in))

        for i in range(int(np.ceil(time_diff/1000))):
            data = pd.concat([data, self.get_pair_data(pair_in, interval_in, startTime=self.datetime_to_unix(begin_time))], axis=0)
            begin_time += pd.Timedelta(minutes=interval_as_int) if interval_mode == 'm' else pd.Timedelta(hours=interval_as_int)

        data.to_csv("zorttiri.csv",index = False)


    def datetime_to_unix(self, dates):
        return (dates - pd.Timestamp("1970-01-01")) // pd.Timedelta('1ms')

    def returnPairFile(self,pair_in,interval_in):
        return f"MarketData/{pair_in}/{pair_in}_{interval_in}.csv"

    def calculateTimeDifferenceAsInterval(self,pair_in,interval_in):
        interval_in_as_int = int(interval_in[:-1])
        file = pd.read_csv(self.returnPairFile(pair_in, interval_in))
